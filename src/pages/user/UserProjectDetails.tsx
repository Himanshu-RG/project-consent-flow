...
import { getProject, uploadPersonConsent } from "@/lib/api/projects";
import { listImages } from "@/lib/api/images";
import { listConsentForms } from "@/lib/api/consent";
import { getPersons, getKnownPersons, updatePerson, promotePerson } from "@/lib/api/persons";
import type {
  ProjectResponse,
  ImageResponse,
  ConsentFormResponse,
...
// ─── Reusable face-detection canvas card ──────────────────────────────────────
const FaceDetectionCard = ({
  ...
  return (
    <div
      className="relative flex-none w-full bg-black flex items-center justify-center snap-center"
...
const UserProjectDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setIsLoading(true);
        const [projectData, imagesData, consentData, personsData] = await Promise.all([
          getProject(id),
          listImages(id),
          listConsentForms(id),
          getPersons(id),
  ...
  const refreshProjectData = async () => {
    if (!id) return;
    try {
      const [imagesData, consentData, personsData] = await Promise.all([
        listImages(id),
        listConsentForms(id),
        getPersons(id),
      ]);
  ...
  const handleConsentUpload = async (personId: string, file: File) => {
    ...
    try {
      ...
      toast({ title: "Consent Uploaded", description: "Consent PDF uploaded and matched." });
      const [personsData, forms] = await Promise.all([getPersons(id), listConsentForms(id)]);
      setPersons(personsData);
  ...
  const handleAssignPid = async () => {
    ...
    try {
      ...
      toast({ title: "Person Assigned", description: `Assigned to ${selectedKnown.name}` });
      const personsData = await getPersons(id!);
      setPersons(personsData);

      // Force modal remount so canvases redraw with fresh detections
  ...
  const handlePromotePerson = async () => {
    ...
    try {
      ...
      toast({ title: "Person Promoted", description: `Added ${promoteName} to global dataset.` });
      const personsData = await getPersons(id!);
      setPersons(personsData);
  ...
  return (
    <AppLayout>
      <div className="space-y-6 animate-in fade-in-0 duration-300">
        ...
        {/* Header — no Edit / Delete buttons for users */}
        ...
        <Tabs defaultValue="images" className="w-full">
                ...
                {images.map((image) => (
                  <Card key={image.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-3 bg-slate-900">
                      ...
                      <p className="text-xs text-slate-400">{image.width} x {image.height}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
          ...
          <TabsContent value="consent" className="space-y-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
              ...
              {/* Users CAN upload consent forms */}
              <Button onClick={() => setAddConsentOpen(true)}>
                <Upload className="mr-2 h-4 w-4" /> Upload Consent Form
              </Button>
            </div>
            {consentForms.length === 0 ? (
              <Card>
              ...
              <div className="space-y-2">
                {consentForms
                  .filter((form) =>
                    form.form_name.toLowerCase().includes(consentSearchQuery.toLowerCase())
                  )
                  .map((form) => (
                    <Card key={form.id}>
                      <CardContent className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {form.signed_date
                                ? `Signed: ${new Date(form.signed_date).toLocaleDateString()}`
                                : "Not signed"}
                            </p>
                          </div>
                        </div>
                        {/* View only — no Delete button for users */}
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() =>
                            window.open(
                              form.file_url?.startsWith("http")
                                ? form.file_url
                                : `http://localhost:8000${form.file_url}`,
                              "_blank"
                            )
                          }
                        >
            ...
            {persons.length === 0 ? (
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-8">
                  ...
                  <p className="text-muted-foreground">No people detected yet. Contact an admin to process images.</p>
                </CardContent>
              </Card>
            ) : (
              <div className="rounded-md border">
                        ...
                        <TableRow key={person.id}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              {/* Upload Consent — if not yet granted */}
                              {person.consent_status !== "granted" && (
                                <Button
                                  variant="outline"
                                  size="sm"
                              ...
                              {/* NOTE: No Delete Person button for user role */}
                            </div>
                          </TableCell>
                        </TableRow>
                      ))}
                ...
                {/* Shared hidden file input for per-person consent upload */}
                <input
            ...
            {images.length === 0 ? (
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-8">
                  ...
                  <p className="text-muted-foreground">No images to redact yet.</p>
                </CardContent>
              </Card>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      ...
                      <div className="aspect-square bg-slate-900 flex items-center justify-center overflow-hidden relative">
                        <img
                          ...
                          onError={(e) => {
                            ...
                            if (parent) {
                              parent.innerHTML =
                                '<div class="flex flex-col items-center gap-2 text-slate-400"><svg class="h-10 w-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span class="text-xs">Not processed</span></div>';
                            }
                          }}
                        />
                        {/* "Run model first" overlay — visible only before processing */}
                        {persons.length === 0 && (
        ...
        >
          <DialogContent className="max-w-2xl w-full p-0 overflow-hidden bg-slate-950 border-slate-800">
            ...
            <div className="relative w-full bg-black">
              ...
              >
                {viewingPerson?.detections && viewingPerson.detections.length > 0 ? (
                  ...
                  <FaceDetectionCard
                    detection={{
                      image_url: viewingPerson.image_url,
                      image_id: viewingPerson.image_id || "",
                      bbox: viewingPerson.bbox || null,
                    }}
            ...
            {/* Manual Assignment (users can also assign PIDs) */}
            <div className="px-5 py-4 bg-slate-900 border-b border-slate-800 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-3">
                  ...
                  <SelectContent className="bg-slate-950 border-slate-700 max-h-[240px]">
                    {knownPersons.map((kp) => (
                      <SelectItem
                        key={kp.pid}
                        value={kp.pid}
                        className="focus:bg-slate-800 focus:text-white text-slate-200 cursor-pointer"
                      >
                        {kp.name} ({kp.pid})
                      </SelectItem>
                    ))}
                ...
                {/* Promote to dataset (available if a bbox/embedding exists) */}
                {viewingPerson?.bbox && (
                  <Button
                    onClick={() => {
                      const defaultName = viewingPerson.name?.startsWith("Unknown Person")
                        ? ""
                        : viewingPerson.name;
            ...
            {promoteDialogOpen && (
              <div className="px-5 py-4 bg-slate-900 border-b border-slate-800 flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="ghost"
                    className="text-slate-400 h-9 px-3"
            ...
            <div className="px-5 py-4 bg-slate-900 grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
              ...
              <div className="flex flex-col gap-1">
                ...
                <Badge
                  variant={
                    viewingPerson?.consent_status === "granted"
                      ? "default"
                      : viewingPerson?.consent_status === "denied"
                      ? "destructive"
                      : "outline"
                  }
                  className={
              ...
              <div className="flex flex-col gap-1">
                ...
                <span
                  className={`font-medium ${
                    (viewingPerson?.confidence ?? 0) >= 0.7
                      ? "text-green-400"
                      : (viewingPerson?.confidence ?? 0) >= 0.55
                      ? "text-yellow-400"
                      : "text-red-400"
                  }`}
                >
